...
cat << "key" > "`pwd`/configure/自定义.prop" && echo "创建文件成功！"
#蓝奏云
...
cat << "key" > "`pwd`/configure/广告奖励.prop" && echo "创建文件成功！"
adsmind.gdtimg.com
c.gdt.qq.com
d.gdt.qq.com
gw.365you.com
...
cat << "key" > "`pwd`/configure/保留奖励.prop" && echo "创建文件成功！"
adsmind.gdtimg.com
c.gdt.qq.com
d.gdt.qq.com
gw.365you.com
...
cat << "key" > "`pwd`/configure/排除列表.prop" && echo "创建文件成功！"
#host屏蔽的域名
#X5内核
...
cat << "key" > "`pwd`/configure/魅族.conf" && echo "创建魅族配置文件成功！"
127.0.0.1 api-flow.flyme.cn
127.0.0.1 bro.flyme.cn
...
EOF
test -f "${file}" && echo "${file} 不存在！！" && exit 1
}
...
echo "${hosts_value}" > "${targetdir}/${value}.conf" && echo "已经输出[ ${targetdir}/${value}.conf ]"
fi
}
